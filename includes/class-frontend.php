...
class OBA_Frontend {
	...
	public function hooks() {
		...
		add_filter( 'woocommerce_locate_template', array( $this, 'locate_auction_template' ), 10, 3 );
	...
	private function build_i18n( $settings ) {
		...
		return array(
			...
			'autobid_prompt_title' => ! empty( $t['autobid_prompt_title'] ) ? $t['autobid_prompt_title'] : __( 'Would you like to set autobid?', 'one-ba-auctions' ),
			'autobid_set_title'    => ! empty( $t['autobid_set_title'] ) ? $t['autobid_set_title'] : __( 'Autobid is set.', 'one-ba-auctions' ),
			'autobid_set'          => ! empty( $t['autobid_set'] ) ? $t['autobid_set'] : __( 'Set autobid', 'one-ba-auctions' ),
			'autobid_edit'         => ! empty( $t['autobid_edit'] ) ? $t['autobid_edit'] : __( 'Edit autobid', 'one-ba-auctions' ),
